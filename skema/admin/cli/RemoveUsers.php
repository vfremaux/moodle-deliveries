...
if ($options['help']) {
    $help =
        ...
        Example:
        \$php moodle_folder/admin/cli/RemoveUsers.php  --rules=<rules> --action=suspend
        ";
    ...
    foreach ($users as $u) {
        ...
        switch ($options['action']) {
            case 'delete': {
                if ($run) {
                    delete_user($u);
                    echo "Deleted [{$u->id}] ({$u->username}) {$u->firstname} {$u->lastname} \n";
                } else {
                    echo "Dryrun : To delete [{$u->id}] ({$u->username}) {$u->firstname} {$u->lastname} \n";
                }
                break;
            }
            ...
            case 'suspend': {
                if ($run) {
                    ...
                    echo "Processed $u->id : suspended\n";
                } else {
                    echo "Dryrun : To suspend [{$u->id}] ({$u->username}) {$u->firstname} {$u->lastname}\n";
                }
                break;
            }
            ...
            case 'unsuspend': {
                if ($run) {
                    ...
                    echo "Processed $u->id : revived\n";
                } else {
                    echo "Dryrun : To revive [{$u->id}] ({$u->username}) {$u->firstname} {$u->lastname} \n";
                }
                break;
            }
        }
...
echo "Done. Processed $ucount /$total ($pc %) users .\n";
exit(0);
